...
public class HistoryController {
	...
	String showOnly(@RequestParam(value = "Show") String type,HttpSession session,
			Model model) {
		
		showGlobal = type;
		if(!session.isNew()) {
			List<Payment> allPayments = (List<Payment>) session.getAttribute("AllPaymentsSession");
			List<String> currCategories = HomePageController.generateCategoriesByType(type, (Map<String, ArrayList<String>>) session.getAttribute("AllCategoriesSession"));
			...
			model.addAttribute("totalAmount",HomePageController.total(allPayments));
	...
	private List<Payment> getPaymentByCategory(String category, List<Payment> payments) {
	...
	String showOnlyCategories(@RequestParam(value = "categories") String category,
			HttpSession session,
			Model model) {
		String type = showGlobal;
		if(!session.isNew()) {
			List<Payment> allPayments = (List<Payment>) session.getAttribute("AllPaymentsSession");
			List<Payment> currPayments = getPaymentByCategory(category, allPayments);
			List<String> currCategories = HomePageController.generateCategoriesByType(type, (Map<String, ArrayList<String>>) session.getAttribute("AllCategoriesSession"));
			model.addAttribute("currCategories",currCategories);
			model.addAttribute("currPayments",currPayments);
			model.addAttribute("totalAmount",HomePageController.total(currPayments));
	...
	String showByDate(@RequestParam(value = "date1") String  firstDate,
		...
		if(!session.isNew()) {
			try{
				...
				List<Payment> allPayments = (List<Payment>) session.getAttribute("AllPaymentsSession");
				List<Payment> payments = getPaymentsBetweenDates(firstDateJava,secondDateJava,allPayments);
				List<String> categories = HomePageController.generateCategoriesByType(showGlobal, (Map<String, ArrayList<String>>) session.getAttribute("AllCategoriesSession"));
	...
	private List<Payment> getPaymentsBetweenDates(LocalDate firstDateJava, LocalDate secondDateJava, List<Payment> allPayments) {
		List<Payment> result = new ArrayList<Payment>();
		result.addAll(allPayments);
		for(Iterator<Payment> itt = result.iterator();itt.hasNext();){
			Payment payment = itt.next();
			if(payment.getDate().isAfter(secondDateJava) || payment.getDate().isBefore(firstDateJava))
					itt.remove();
		}
		return result;
	}
	private LocalDate stringToLocalDate(String date) throws IllegalArgumentException,DateTimeParseException{
	...
	String sortByDate(Model model,HttpSession session) {
		...
		if(!session.isNew()) {
			String type = showGlobal;
			List<Payment> payments = (List<Payment>) session.getAttribute("AllPaymentsSession");
			List<String> categories = HomePageController.generateCategoriesByType(type, (Map<String, ArrayList<String>>) session.getAttribute("AllCategoriesSession"));
	...
	public static void modifyByDate(String date, List<Payment> payments) throws IllegalArgumentException{
		LocalDate dateJava;
		try
		{
			DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy");
